...
if (cluster.isPrimary) {
    ...
    const limiter = rateLimit({
        windowMs: 60 * 20 * 1000,
        max: 5,
        message: 'Limit exceeded please try after 20 minute',
    ...
    server.use((req, res, next) => {
        ...
        if (val === "?1") {
            return res.status(400).json({ message: "This site not allowed for mobile phone till now its in development mode of responsive site to access this site dont use mobile" })
        } else {
            next()
        }
    })
